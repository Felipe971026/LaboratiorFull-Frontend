...
export const Kardex: React.FC = () => {
      ...
      for (const supply of supplies) {
        ...
        if (supply.currentStock < 0) {
          try {
            ...
            await addDoc(collection(db, 'kardexEntries'), {
              supplyId: supply.id,
  ...
  const getSupplyBatches = (supplyId: string): BatchInfo[] => {
    ...
    const batchMap: Record<string, { balance: number, exp: string }> = {};

    supplyEntries.forEach(entry => {
      if (!entry.batch) return;
      if (!batchMap[entry.batch]) {
        batchMap[entry.batch] = { balance: 0, exp: entry.expirationDate || '' };
      }
      if (entry.type === 'input') {
  ...
  const handleDeleteSupply = async () => {
    ...
    try {
      ...
      try {
        ...
        await addDoc(collection(db, 'kardexEntries'), entryData);
      } catch (auditError) {
